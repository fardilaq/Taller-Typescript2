...
const tbody = document.querySelector("#series-body");
...
const avgEl = document.querySelector("#average-seasons");
if (avgEl)
    avgEl.textContent = promedio.toFixed(2);
const detail = document.querySelector("#series-detail");
tbody.addEventListener("click", (ev) => {
    const link = ev.target.closest("a.serie-link");
    if (!link)
        return;
    ev.preventDefault();
...
if (series.length)
    detail.innerHTML = series[0].toCardHTML();
